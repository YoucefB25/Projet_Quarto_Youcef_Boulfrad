...
\hypersetup{
  pdftitle={K-Nearest Neighbors (KNN) - One-Versus-All (OVA)},
  colorlinks=true,
  linkcolor={blue},
  filecolor={Maroon},
  citecolor={Blue},
...
\title{K-Nearest Neighbors (KNN) - One-Versus-All (OVA)}
\author{}
\date{}
...
Dans l'approche \textbf{One-Versus-All (OVA)}, un modèle KNN est
entraîné pour chaque classe, en distinguant chaque classe contre toutes
les autres regroupées.
...
\NormalTok{    knn\_ova }\OperatorTok{=}\NormalTok{ OneVsRestClassifier(KNeighborsClassifier(n\_neighbors}\OperatorTok{=}\NormalTok{k))}
\NormalTok{    knn\_ova.fit(X\_train, y\_train)}
\NormalTok{    acc }\OperatorTok{=}\NormalTok{ accuracy\_score(y\_val, knn\_ova.predict(X\_val))}
...
\NormalTok{plt.title(}\StringTok{"Impact du nombre de voisins sur la performance de KNN (OVA)"}\NormalTok{)}
\NormalTok{plt.legend()}
\NormalTok{plt.show()}

\CommentTok{\# 🏆 Modèle final avec le meilleur hyperparamètre}
\NormalTok{final\_model }\OperatorTok{=}\NormalTok{ OneVsRestClassifier(KNeighborsClassifier(n\_neighbors}\OperatorTok{=}\NormalTok{best\_k))}
...
\pandocbounded{\includegraphics[keepaspectratio]{knn_ova_files/figure-pdf/cell-2-output-1.pdf}}
