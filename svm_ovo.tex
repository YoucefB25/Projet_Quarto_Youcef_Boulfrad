...
\hypersetup{
  pdftitle={Support Vector Machines (SVM) - One-Versus-One (OVO)},
  colorlinks=true,
  linkcolor={blue},
  filecolor={Maroon},
  citecolor={Blue},
...
\title{Support Vector Machines (SVM) - One-Versus-One (OVO)}
\author{}
\date{}
...
l'approche \textbf{One-Versus-One (OVO)} entraîne un SVM pour chaque
paire de classes, ce qui permet une meilleure séparation lorsque les
classes sont bien distinctes.
...
\NormalTok{    model }\OperatorTok{=}\NormalTok{ OneVsOneClassifier(SVC(kernel}\OperatorTok{=}\StringTok{\textquotesingle{}rbf\textquotesingle{}}\NormalTok{, C}\OperatorTok{=}\NormalTok{C))}
...
\NormalTok{plt.title(}\StringTok{"Impact de la régularisation sur la performance du SVM (OVO)"}\NormalTok{)}
\NormalTok{plt.legend()}
\NormalTok{plt.show()}

\CommentTok{\# 🏆 Modèle final avec le meilleur hyperparamètre}
\NormalTok{final\_model }\OperatorTok{=}\NormalTok{ OneVsOneClassifier(SVC(kernel}\OperatorTok{=}\StringTok{\textquotesingle{}rbf\textquotesingle{}}\NormalTok{, C}\OperatorTok{=}\NormalTok{best\_C))}
...
\pandocbounded{\includegraphics[keepaspectratio]{svm_ovo_files/figure-pdf/cell-2-output-1.pdf}}
