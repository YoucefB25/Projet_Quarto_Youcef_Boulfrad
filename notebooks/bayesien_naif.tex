...
\hypersetup{
  pdftitle={Classifieur Bayésien Naïf},
  colorlinks=true,
  linkcolor={blue},
  filecolor={Maroon},
  citecolor={Blue},
...
\title{Classifieur Bayésien Naïf}
\author{}
\date{}
...
Le classificateur \textbf{Bayésien Naïf} repose sur le \textbf{théorème
de Bayes} et l'hypothèse d'indépendance conditionnelle entre les
variables explicatives. Il est particulièrement efficace pour les
problèmes de classification textuelle et fonctionne bien même avec peu
...
\tightlist
\item
  \textbf{\texttt{var\_smoothing}} : Ce paramètre permet d'ajouter un
  lissage aux variances estimées pour éviter les divisions par zéro. Il
  est sélectionné en fonction de la précision sur l'ensemble de
  validation.
\end{itemize}
...
\subsection{\texorpdfstring{Recherche du meilleur
\texttt{var\_smoothing} et
évaluation}{Recherche du meilleur var\_smoothing et évaluation}}\label{recherche-du-meilleur-var_smoothing-et-uxe9valuation}
...
\CommentTok{\# 🎯 Recherche du meilleur hyperparamètre var\_smoothing}
\NormalTok{var\_smoothing\_values }\OperatorTok{=}\NormalTok{ np.logspace(}\OperatorTok{{-}}\DecValTok{9}\NormalTok{, }\DecValTok{0}\NormalTok{, }\DecValTok{10}\NormalTok{)}
\NormalTok{val\_accuracies }\OperatorTok{=}\NormalTok{ []}

\ControlFlowTok{for}\NormalTok{ smoothing }\KeywordTok{in}\NormalTok{ var\_smoothing\_values:}
\NormalTok{    gnb }\OperatorTok{=}\NormalTok{ GaussianNB(var\_smoothing}\OperatorTok{=}\NormalTok{smoothing)}
\NormalTok{    gnb.fit(X\_train, y\_train)}
\NormalTok{    acc }\OperatorTok{=}\NormalTok{ accuracy\_score(y\_val, gnb.predict(X\_val))}
\NormalTok{    val\_accuracies.append((smoothing, acc))}

\CommentTok{\# Sélection du meilleur hyperparamètre}
\NormalTok{best\_smoothing, best\_val\_acc }\OperatorTok{=} \BuiltInTok{max}\NormalTok{(val\_accuracies, key}\OperatorTok{=}\KeywordTok{lambda}\NormalTok{ x: x[}\DecValTok{1}\NormalTok{])}


\CommentTok{\# 📈 Affichage du graphique}
\NormalTok{plt.figure(figsize}\OperatorTok{=}\NormalTok{(}\DecValTok{8}\NormalTok{, }\DecValTok{6}\NormalTok{))}
\NormalTok{plt.plot(var\_smoothing\_values, [acc }\ControlFlowTok{for}\NormalTok{ smoothing, acc }\KeywordTok{in}\NormalTok{ val\_accuracies], marker}\OperatorTok{=}\StringTok{\textquotesingle{}o\textquotesingle{}}\NormalTok{, linestyle}\OperatorTok{=}\StringTok{\textquotesingle{}dashed\textquotesingle{}}\NormalTok{, label}\OperatorTok{=}\StringTok{"Validation"}\NormalTok{)}
\NormalTok{plt.xscale(}\StringTok{\textquotesingle{}log\textquotesingle{}}\NormalTok{)}
\NormalTok{plt.xlabel(}\StringTok{"Valeur de var\_smoothing"}\NormalTok{)}
\NormalTok{plt.ylabel(}\StringTok{"Précision sur validation"}\NormalTok{)}
\NormalTok{plt.title(}\StringTok{"Optimisation du paramètre var\_smoothing pour GaussianNB"}\NormalTok{)}
\NormalTok{plt.legend()}
\NormalTok{plt.show()}

\CommentTok{\# 🏆 Modèle final avec le meilleur hyperparamètre}
\NormalTok{final\_model }\OperatorTok{=}\NormalTok{ GaussianNB(var\_smoothing}\OperatorTok{=}\NormalTok{best\_smoothing)}
...
\BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f"}\CharTok{\textbackslash{}n}\SpecialStringTok{🔹 Meilleur var\_smoothing sur l\textquotesingle{}échantillon de validation : }\SpecialCharTok{\{}\NormalTok{best\_smoothing}\SpecialCharTok{:.1e\}}\SpecialStringTok{"}\NormalTok{)}
...
\pandocbounded{\includegraphics[keepaspectratio]{bayesien_naif_files/figure-pdf/cell-2-output-1.pdf}}
