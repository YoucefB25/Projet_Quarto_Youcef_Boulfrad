...
\hypersetup{
  pdftitle={Arbre de Décision - CART},
  colorlinks=true,
  linkcolor={blue},
  filecolor={Maroon},
  citecolor={Blue},
...
L'algorithme \textbf{CART} (\textbf{Classification and Regression
Trees}) est un modèle d'apprentissage supervisé qui construit un
\textbf{arbre de décision} en divisant l'espace des caractéristiques en
...
\tightlist
\item
  \textbf{Profondeur maximale de l'arbre (\texttt{max\_depth})} :
  sélectionnée en fonction de la précision sur l'ensemble de validation.
\end{itemize}
...
\subsection{\texorpdfstring{Recherche de la meilleure
\texttt{max\_depth} et
évaluation}{Recherche de la meilleure max\_depth et évaluation}}\label{recherche-de-la-meilleure-max_depth-et-uxe9valuation}
...
\CommentTok{\# 🎯 Recherche du meilleur hyperparamètre max\_depth}
\NormalTok{depth\_range }\OperatorTok{=} \BuiltInTok{range}\NormalTok{(}\DecValTok{1}\NormalTok{, }\DecValTok{30}\NormalTok{)}
\NormalTok{val\_accuracies }\OperatorTok{=}\NormalTok{ []}

\ControlFlowTok{for}\NormalTok{ depth }\KeywordTok{in}\NormalTok{ depth\_range:}
\NormalTok{    tree }\OperatorTok{=}\NormalTok{ DecisionTreeClassifier(max\_depth}\OperatorTok{=}\NormalTok{depth, random\_state}\OperatorTok{=}\DecValTok{42}\NormalTok{)}
\NormalTok{    tree.fit(X\_train, y\_train)}
\NormalTok{    acc }\OperatorTok{=}\NormalTok{ accuracy\_score(y\_val, tree.predict(X\_val))}
\NormalTok{    val\_accuracies.append((depth, acc))}

\CommentTok{\# Sélection de la meilleure profondeur d\textquotesingle{}arbre}
\NormalTok{best\_depth, best\_val\_acc }\OperatorTok{=} \BuiltInTok{max}\NormalTok{(val\_accuracies, key}\OperatorTok{=}\KeywordTok{lambda}\NormalTok{ x: x[}\DecValTok{1}\NormalTok{])}


\CommentTok{\# 📈 Affichage du graphique}
\NormalTok{plt.figure(figsize}\OperatorTok{=}\NormalTok{(}\DecValTok{8}\NormalTok{, }\DecValTok{6}\NormalTok{))}
\NormalTok{plt.plot(depth\_range, [acc }\ControlFlowTok{for}\NormalTok{ depth, acc }\KeywordTok{in}\NormalTok{ val\_accuracies], marker}\OperatorTok{=}\StringTok{\textquotesingle{}o\textquotesingle{}}\NormalTok{, linestyle}\OperatorTok{=}\StringTok{\textquotesingle{}dashed\textquotesingle{}}\NormalTok{, label}\OperatorTok{=}\StringTok{"Validation"}\NormalTok{)}
\NormalTok{plt.xlabel(}\StringTok{"Profondeur de l\textquotesingle{}arbre"}\NormalTok{)}
\NormalTok{plt.ylabel(}\StringTok{"Précision sur validation"}\NormalTok{)}
\NormalTok{plt.title(}\StringTok{"Impact de la profondeur de l\textquotesingle{}arbre sur la performance de CART"}\NormalTok{)}
\NormalTok{plt.legend()}
\NormalTok{plt.show()}

\CommentTok{\# 🏆 Modèle final avec la meilleure profondeur}
\NormalTok{final\_model }\OperatorTok{=}\NormalTok{ DecisionTreeClassifier(max\_depth}\OperatorTok{=}\NormalTok{best\_depth, random\_state}\OperatorTok{=}\DecValTok{42}\NormalTok{)}
...
\BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f"}\CharTok{\textbackslash{}n}\SpecialStringTok{🔹 Meilleure profondeur de l\textquotesingle{}arbre sur l’échantillon de validation : }\SpecialCharTok{\{}\NormalTok{best\_depth}\SpecialCharTok{\}}\SpecialStringTok{"}\NormalTok{)}
...
\pandocbounded{\includegraphics[keepaspectratio]{arbre_decision_files/figure-pdf/cell-2-output-1.pdf}}
