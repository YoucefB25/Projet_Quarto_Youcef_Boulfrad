...
\hypersetup{
  pdftitle={KNN - K-Nearest Neighbors},
  colorlinks=true,
  linkcolor={blue},
  filecolor={Maroon},
  citecolor={Blue},
...
\NormalTok{    acc }\OperatorTok{=}\NormalTok{ accuracy\_score(y\_val, knn.predict(X\_val))}
...
\NormalTok{plt.title(}\StringTok{"Impact du nombre de voisins sur la performance de KNN"}\NormalTok{)}
\NormalTok{plt.legend()}
\NormalTok{plt.show()}

\CommentTok{\# 🏆 Modèle final avec le meilleur hyperparamètre}
\NormalTok{final\_model }\OperatorTok{=}\NormalTok{ KNeighborsClassifier(n\_neighbors}\OperatorTok{=}\NormalTok{best\_k)}
...
\pandocbounded{\includegraphics[keepaspectratio]{knn_multiclasse_files/figure-pdf/cell-2-output-1.pdf}}
