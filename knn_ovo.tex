...
\hypersetup{
  pdftitle={K-Nearest Neighbors (KNN) - One-Versus-One (OVO)},
  colorlinks=true,
  linkcolor={blue},
  filecolor={Maroon},
  citecolor={Blue},
...
\title{K-Nearest Neighbors (KNN) - One-Versus-One (OVO)}
\author{}
\date{}
...
Le \textbf{K-Nearest Neighbors (KNN)} est un algorithme de
classification basé sur la proximité des données dans un espace
multidimensionnel. Il attribue une classe à un point en fonction des
\textbf{K voisins les plus proches}.

Dans l'approche \textbf{One-Versus-One (OVO)}, un modèle KNN est
entraîné pour chaque paire de classes. Pour \textbf{7 classes}, cela
signifie que \textbf{21 modèles} sont entraînés.
...
\tightlist
\item
  \textbf{Nombre de voisins (\texttt{k})} : déterminé en fonction de la
...
\subsection{\texorpdfstring{Recherche du meilleur \texttt{k} et
évaluation}{Recherche du meilleur k et évaluation}}\label{recherche-du-meilleur-k-et-uxe9valuation}
...
\CommentTok{\# 🎯 Recherche du meilleur hyperparamètre k}
\NormalTok{k\_values }\OperatorTok{=} \BuiltInTok{range}\NormalTok{(}\DecValTok{1}\NormalTok{, }\DecValTok{51}\NormalTok{, }\DecValTok{2}\NormalTok{)}
\NormalTok{val\_accuracies }\OperatorTok{=}\NormalTok{ []}

\ControlFlowTok{for}\NormalTok{ k }\KeywordTok{in}\NormalTok{ k\_values:}
\NormalTok{    knn\_ovo }\OperatorTok{=}\NormalTok{ OneVsOneClassifier(KNeighborsClassifier(n\_neighbors}\OperatorTok{=}\NormalTok{k))}
\NormalTok{    knn\_ovo.fit(X\_train, y\_train)}
\NormalTok{    acc }\OperatorTok{=}\NormalTok{ accuracy\_score(y\_val, knn\_ovo.predict(X\_val))}
\NormalTok{    val\_accuracies.append((k, acc))}

\CommentTok{\# Sélection du meilleur hyperparamètre}
\NormalTok{best\_k, best\_val\_acc }\OperatorTok{=} \BuiltInTok{max}\NormalTok{(val\_accuracies, key}\OperatorTok{=}\KeywordTok{lambda}\NormalTok{ x: x[}\DecValTok{1}\NormalTok{])}

\CommentTok{\# 📈 Affichage du graphique}
\NormalTok{plt.figure(figsize}\OperatorTok{=}\NormalTok{(}\DecValTok{8}\NormalTok{, }\DecValTok{6}\NormalTok{))}
\NormalTok{plt.plot(k\_values, [acc }\ControlFlowTok{for}\NormalTok{ k, acc }\KeywordTok{in}\NormalTok{ val\_accuracies], marker}\OperatorTok{=}\StringTok{\textquotesingle{}o\textquotesingle{}}\NormalTok{, linestyle}\OperatorTok{=}\StringTok{\textquotesingle{}dashed\textquotesingle{}}\NormalTok{, label}\OperatorTok{=}\StringTok{"Validation"}\NormalTok{)}
\NormalTok{plt.xlabel(}\StringTok{"Nombre de voisins (k)"}\NormalTok{)}
\NormalTok{plt.ylabel(}\StringTok{"Précision sur validation"}\NormalTok{)}
\NormalTok{plt.title(}\StringTok{"Impact du nombre de voisins sur la performance de KNN (OVO)"}\NormalTok{)}
\NormalTok{plt.legend()}
\NormalTok{plt.show()}

\CommentTok{\# 🏆 Modèle final avec le meilleur hyperparamètre}
\NormalTok{final\_model }\OperatorTok{=}\NormalTok{ OneVsOneClassifier(KNeighborsClassifier(n\_neighbors}\OperatorTok{=}\NormalTok{best\_k))}
...
\BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f"}\CharTok{\textbackslash{}n}\SpecialStringTok{🔹 Meilleur nombre de voisins (k) sur l\textquotesingle{}échantillon de validation : }\SpecialCharTok{\{}\NormalTok{best\_k}\SpecialCharTok{\}}\SpecialStringTok{"}\NormalTok{)}
...
\pandocbounded{\includegraphics[keepaspectratio]{knn_ovo_files/figure-pdf/cell-2-output-1.pdf}}
